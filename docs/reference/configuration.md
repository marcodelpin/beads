...
| `dolt.open-retry-budget` | — | — | `0` (off) | How long to keep **retrying** the connectivity probe when opening a Dolt server bd does not manage (duration like `30s`/`2m`, or a bare number of seconds). The first probe is unchanged and is never shortened by this budget, so a value too small to buy a retry degrades to the fail-fast open. `0` or unset keeps today's fail-fast open; an explicit `0` in the project being opened (`.beads/config.local.yaml` or `.beads/config.yaml`, either YAML spelling) also overrides a wider default. Resolution is scoped to that project plus the user-level `config.yaml`, never another project's settings, and "that project" means its `.beads` directory even when its Dolt data lives elsewhere (shared-server mode, a custom `dolt_data_dir`). Ignored in embedded mode (any workspace whose `.beads/metadata.json` does not select a server, not only one that says `dolt_mode: embedded`), in socket mode, for a proxied server, and for a bd-managed localhost server — see [Troubleshooting](/reference/troubleshooting#server-restarts-make-commands-fail-instantly) |
