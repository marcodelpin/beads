...
the flags appear in the command line.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for i, id := range resolvedIDs {
			...
			if res.Err != nil {
				if errors.Is(res.Err, errCloseSpooled) {
					// Fork seam (GH#4379, internal/spool): the close is QUEUED,
					// not applied - skip the success side effects (audit log,
					// molecule auto-close, closedCount that gates the
					// --suggest-next/--claim-next cascade) until the replay
					// actually lands it. Report an honest queued outcome.
					if !jsonOutput {
						fmt.Printf("%s Queued close for replay (server unreachable): %s\n",
							ui.RenderWarn("!"), formatFeedbackID(id, issueTitleOrEmpty(results[i].Issue)))
					}
					continue
				}
