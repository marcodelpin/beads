...
func runLabelAdd(ctx context.Context, args []string, replace bool) error {
	...
	if replace {
		return applyLabelAddReplace(ctx, issueIDs, labels)
	}
	return applyLabelEdit(ctx, issueIDs, labels, labelOperationAdded)
}

// applyLabelAddReplace is 'bd label add --replace': in a configured exclusive
// namespace (labels.exclusive-prefixes, bd-7u5ki), swap out any OTHER label
...
func applyLabelAddReplace(ctx context.Context, issueIDs []string, labels []string) error {
	...
	settings, err := openWorkspaceConfig("label add --replace requires reading labels.exclusive-prefixes")
	if err != nil {
		return HandleErrorRespectJSON("%v", err)
	}
	ctx, err = issueOpsContext(ctx)
	...
	raw, err := settings.GetSetting(ctx, issueops.GetSettingRequest{Key: labelns.ConfigKey})
	if err != nil {
		return HandleErrorRespectJSON("reading %s: %v", labelns.ConfigKey, err)
	}
	...
	return reportLabelEdit(issueIDs, labels, labelOperationAdded, jsonOutput)
}

// conflictingExclusiveLabels returns the labels in current that occupy the
// same exclusive namespace as one of the incoming labels but are not
// themselves one of them - the set applyLabelAddReplace must remove before
// the add, deduplicated and order-independent.
func conflictingExclusiveLabels(prefixes, incoming, current []string) []string {
	var toRemove []string
