...
func TestClientServerShareFilesystem_GH3523(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// clientServerShareFilesystem now consults metadata.json via
			// FindBeadsDir (GH#3545 precedence), which walks UP from the
			// cwd - without this isolation it finds the repo's own .beads
			// and reads the real fleet server host.
			t.Chdir(t.TempDir())
			if tt.envHost == "\x00" {
