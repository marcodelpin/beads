...
func applyUpdateProxiedOne(ctx context.Context, id string, in *updateInput) (*types.Issue, *updateIDFailure, error) {
	...
	// Patch building is pure reshaping now: claim, guard and serialization
	// verdicts arise at the MUTATION and route through proxiedUpdateFailure,
	// which already carries the fork's claim-failure and GuardMismatch
	// reporting. The fork's old 4-value error branch here described a phase
	// that no longer performs writes.
