...
func closeDirectRun(ctx context.Context, batches []closeDirectBatch, argCount int, session string, force bool, claimStore storage.DoltStorage, claimNext *issueops.ReadyRequest) ([]*issueops.CloseOutcome, *types.IssueWithCounts) {
	...
	for _, batch := range batches {
		...
		if err != nil {
			// Fork seam (GH#4379, internal/spool): a TRANSIENT batch failure
			// means the server never saw any of these closes - queue each id
			// for offline replay instead of refusing it. Permanent errors
			// (incl. the typed close vocabulary) pass through untouched.
			if spool.IsTransientErr(err) && spoolCloseOutcomes(ctx, batch, session, err, outcomes) {
				continue
			}
			for _, item := range batch.items {
