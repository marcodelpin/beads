...
func gitAddFile(path string) error {
	...
	// Fork: keep execx.GitCommandContext (hidden-window git spawn) rather than
	// bare exec.CommandContext; the argument guard above is orthogonal to it.
	cmd := execx.GitCommandContext(ctx, "add", "--", filepath.Base(path))
