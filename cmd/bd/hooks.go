...
func resetHooksPathIfBeadsManaged() error {
	...
	if out, err := cmd.Output(); err == nil {
		...
		if doctor.IsBeadsManagedHooksPath(repoRoot, hooksPath) {
			unsetCmd := execx.GitCommand("config", "--unset", "core.hooksPath")
	...
	getRoleCmd := execx.GitCommand("config", "--get", "beads.role")
	getRoleCmd.Dir = repoRoot
	if _, err := getRoleCmd.Output(); err == nil {
		roleCmd := execx.GitCommand("config", "--unset", "beads.role")
