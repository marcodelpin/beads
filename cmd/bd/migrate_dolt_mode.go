...
func runMigrateFromProxiedServer(dryRun bool, shared bool) error {
	...
	if j.Phase == migrateTargetConfigured {
		...
		// Release the two control-file locks before the purge deletes the files
		// they are held on (bda-9s2): Windows refuses to remove an open file.
		// serverLock is deliberately still held -- it guards a path under
		// serverStateDir that this purge does not touch. Unlock is not
		// idempotent, so the flag clears the deferred release; a resumed
		// migration that re-enters this phase finds the flags already false.
		if proxyLockHeld {
			proxyLock.Unlock()
			proxyLockHeld = false
		}
