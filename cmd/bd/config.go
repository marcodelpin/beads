...
var configSetCmd = &cobra.Command{
	...
	RunE: func(_ *cobra.Command, args []string) error {
		...
		// labels.exclusive-prefixes validation (like status.custom's) lives in
		// workapi.ValidateSettingWrite, which every WorkspaceConfig backend
		// calls - so the direct route below and the proxied route both refuse
		// a bad value, instead of only the door that happens to duplicate the
		// check.
