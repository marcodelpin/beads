...
func importIssuesCore(ctx context.Context, _ string, store storage.DoltStorage, issues []*types.Issue, opts ImportOptions) (*ImportResult, error) {
	...
	batchOpts := storage.BatchCreateOptions{
		...
		// Exclusive label namespaces (bd-7u5ki): import replays history that
		// may predate the labels.exclusive-prefixes config, so violations are
		// reported through the callback and the labels are kept as-is rather
		// than failing the whole import. Set on batchOpts (not passed
		// per-call) so BOTH the small-import and the chunked path warn the
		// same way.
	...
	return assembleImportResult(issues, staleSkippedIDs, changePlan, staleRejectedSet, skippedDependencies, exclusiveLabelConflicts), nil
}
...
// is attributed.
func assembleImportResult(issues []*types.Issue, staleSkippedIDs []string, changePlan importChangePlan, staleRejectedSet map[string]struct{}, skippedDependencies []string, exclusiveLabelConflicts []string) *ImportResult {
