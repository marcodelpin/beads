...
func runCreateProxiedSingle(_ *cobra.Command, ctx context.Context, in createInput) error {
	...
	//
	// Build-inside-the-attempt is the role's job now: Lifecycle.Create clones
	// the request per RunTxResult attempt, so a serialization-failure retry
	// cannot reuse a struct a failed attempt mutated
