...
// ConfigUseCase panics BY NAME rather than by promoted-nil SIGSEGV: the
// assertion is the same (a command whose lookup failed must not touch config)
// but the failure now says which surface was touched instead of dumping a
// segfault trace through an autogenerated wrapper (bda-t54v).
func (u lookupOnlyUOW) ConfigUseCase() domain.ConfigUseCase {
	panic("lookup-only UOW: command touched ConfigUseCase after a failed lookup")
}
