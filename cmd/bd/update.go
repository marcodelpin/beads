...
pointless).`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, id := range args {
			...
			var updateResult issueops.UpdateResult
			mutate := func() error {
				var merr error
				updateResult, merr = runCommandUpdateMutation(opsCtx, ops, commandUpdateMutation{
					actor:            actor,
			...
			// Fork seam (GH#4520, internal/spool): non-claim updates keep the
			// offline write-spool around the atomic mutation. Safe by
			// construction - writeWithSpool only queues when
			// spool.IsTransientErr(err); a guard refusal is not transient, so
			// it returns the error instead of silently queueing a refused
			// write. A --claim is never spooled: the CAS is time-sensitive and
			// this route has always failed it loud on an unreachable server.
			// The replay payload carries the FIELD updates only - label ops and
			// the reparent were never spooled on this route.
			var spooled bool
			var updateErr error
			if claimFlag {
				updateErr = mutate()
			} else {
				spoolable := make(map[string]interface{}, len(updates))
				for k, v := range updates {
					if k != "add_labels" && k != "remove_labels" && k != "set_labels" && k != "parent" {
						spoolable[k] = v
					}
				}
				var res spoolOutcome
				res, updateErr = writeWithSpool(ctx, "update",
			...
			if updateErr != nil {
				fmt.Fprintf(os.Stderr, "Error updating %s: %v\n", id, updateErr)
				// Not recordFailure(): that fork helper cannot carry GuardMismatch,
				// and reportUpdateFailures needs it to exit ExitGuardMismatch rather
				// than a generic 1 when every failure is a precondition refusal.
