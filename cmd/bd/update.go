	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, id := range args {
			...
			if len(regularUpdates) > 0 {
				// Guard/CAS path (upstream bd-wsqvw): with --if-assignee/--if-status
				// present, route through the checked write so a stale precondition
				// refuses atomically with a typed mismatch error and MUST surface as a
				// non-zero exit - never collapsed to success.
				// LAYERED inside the fork's write-spool (GH#4520): res.Spooled is read
				// by the shared tail below, so the spool wrapper cannot be dropped here.
				// Safe by construction - writeWithSpool only queues when
				// spool.IsTransientErr(err); a guard refusal is not transient, so it
				// returns the error instead of silently queueing a refused write.
				res, err := writeWithSpool(ctx, "update",
					...
					func() error {
						if ifAssignee != nil || ifStatus != nil {
							return issueStore.UpdateIssueChecked(ctx, result.ResolvedID, regularUpdates, actor,
				...
				if err != nil {
					...
					failures = append(failures, updateIDFailure{
						ID:            id,
						Error:         fmt.Sprintf("updating issue: %v", err),
						GuardMismatch: isGuardMismatch(err),
					})
					closeIfUnmutated(result)
					continue
				}
