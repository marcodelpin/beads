...
func RunAudit_issue_lifecycle(t *testing.T, f Factory) {
	...
	t.Run("CloseCheckedReportsUnchanged", func(t *testing.T) { testAuditCloseCheckedReportsUnchanged(t, f) })
...
// CloseIssueChecked surfaces the already-closed no-op that CloseIssue keeps
// silent (GH#4816): the first close reports Unchanged=false, a re-close
// reports Unchanged=true with the GH#4025 contract intact (nil error, first
// reason wins, no second EventClosed), and a missing ID is still ErrNotFound.
func testAuditCloseCheckedReportsUnchanged(t *testing.T, f Factory) {
	s := f(t)
	must(t, s.CreateIssue(ctx(), withDefaults(&types.Issue{ID: "cwr-1", Title: "T", Status: types.StatusOpen}), "a"))

	res, err := s.CloseIssueChecked(ctx(), "cwr-1", "a", storage.CloseIssueOptions{Reason: "first", Session: "s1"})
	must(t, err)
	if res.Unchanged {
		t.Fatalf("first close: result = %+v, want Unchanged=false", res)
	}

	res, err = s.CloseIssueChecked(ctx(), "cwr-1", "b", storage.CloseIssueOptions{Reason: "second", Session: "s2"})
	if err != nil {
		t.Fatalf("re-close: err = %v, want nil (idempotent per GH#4025)", err)
	}
	if !res.Unchanged {
		t.Errorf("re-close: result = %+v, want Unchanged=true", res)
	}
	...
	if _, err := s.CloseIssueChecked(ctx(), "cwr-missing", "a", storage.CloseIssueOptions{Reason: "x", Session: "s"}); !errors.Is(err, storage.ErrNotFound) {
...
//
// This is upstream's own #5191 re-specification (testAuditReopenEventSemantics,
// b92442d1a), harvested verbatim: the fork merge fdaab2d34 had resurrected the
// pre-#5191 ReopenOnOpenMintsEvent variant - retired upstream in #5453 and
// re-specified in #5191 - whose mints-event expectation contradicts the
// documented issueops contract (reopen only acts on done-category statuses)
// and was born red on the Dolt backend (bda-wlv8).
