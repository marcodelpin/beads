...
//
// IT RECURSES UNWRAPPED, for the reason hook_sweeper.go gives at length and
// which applies here unchanged: internal/hooks publishes on_create, on_update
// and on_close, and a deletion is none of them. Firing on_update for a row that
// no longer exists would hand a user's script an id it cannot read back, and
// the result this role returns carries counts rather than rows, so there is no
// issue to hand a hook script even if one existed.
//
...
// failure mode wearing a feature's clothes. The direct CLI route this role
// replaces fired no hook for those updates either.
//
