...
// refusal is a suggestion. storage.DoltStorage publishes methods, not
// transactions, so a body written against it could only check-then-act. The two
// Dolt-backed stores share these bodies and differ only in how they reach a
// transaction, so they are ONE vote and the unit-of-work provider is the
// second. There is nothing here a front door could construct — both functions
// take a transaction — so no depguard entry is needed to keep them out of
// cmd/bd.
...
// workapi.ValidateBootstrapRequest. The accessors validate BEFORE opening a
...
// shared database is not hypothetical — it is the ordinary case for a
// bts-provisioned server and for two rigs pointed at one Dolt database — and a
// refusal decided outside the transaction that writes is one both racers pass.
//
// The per-clone bookkeeping `bd init` seeds alongside the identity — the
// fingerprints, the synced-at marker, the recorded binary version — is not here
...
// ask about, so the two cannot come to disagree about what "identified" means.
//
// Both reads happen even when the first already decides the outcome: the
// verifier publishes the pair, and the refusal names both values.
