...
func RemovedBackendDetail(backend string) string {
	return fmt.Sprintf("%s; the configured %s database was not opened or modified; export it with a bd version that supports %s, then follow bd help init-safety to reinitialize with Dolt or SQLite and import the exported data", RemovedBackendRationale, backend, backend)
}
...
func UnknownBackendError(backend string) error {
	return fmt.Errorf("storage backend %q in metadata.json is not recognized or supported; %s; supported backends are %q and %q; fix or restore metadata.json and retry", backend, BackendNotOpenedGuarantee, BackendDolt, BackendSQLite)
}
