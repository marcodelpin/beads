...
//
// WHAT IS NOT HERE IS THE POINT, exactly as in reads.go. No filter is built,
// no assignee predicate is assembled, no ready-work query is issued and no
// status fold is performed: all of that is inside issueops.StatsReporter's
// implementation, which `bd status` reaches through the same accessor on both
// of its routes. The fold in particular is the thing worth keeping out of a
// handler — it is where "your work" is DEFINED, and a second copy of it here
// would be a second definition that nothing compares against the first.
//
// WHICH METHOD IS TRANSPORT'S CALL, and it is the only decision this file
// makes about meaning: an absent `assignee` is the workspace-wide question, a
// present one is the actor question, and an EMPTY one is neither, so it is
// refused here rather than passed down. The role would refuse it too
// (ErrValidation), but a 400 that names the parameter is what a client can act
// on, and the document says the workspace-wide question is asked by omitting
// the parameter rather than by emptying it.
//
// EMPTY MEANS TRIMMED-EMPTY, because that is what the role means by it
// (workapi.ValidateStatsAssignee trims). Checking the raw string let
// `?assignee=%20` past the guard, down to a role refusal this file had no
// classifier for, and out as a 500 with an operator alert attached — for a
// trailing space in somebody's script.
...
func (s *Server) handleStats(w http.ResponseWriter, r *http.Request) {
	...
	if assignee != "" {
		...
		// says the parameter is ignored here; this is where it is ignored.
	...
	writeJSON(w, apigen.StatsResponse{
		...
		// and a flag echoing the request would then tell a client the scan was
