...
// internal/workapi/storecounter are — see those packages' docs. A constructor
// sitting in internal/workapi would be a one-line drop-in for
// store.WorkspaceConfig() from any front door, and one that silently skips the
// decorators, because a decorator adds its layer in its own accessor. Down here
// the only importers are the two Dolt store packages and the
...
//
// THE PROJECTION IS THE STORE'S, not this body's, and that is deliberate: both
// store backends' SetConfig already rewrite custom_statuses / custom_types
// inside the same retryable transaction as the row, and they additionally drop
// the process-local caches those tables feed. Re-implementing either here would
// put the projection outside that transaction — a row that landed and a table
// that did not is precisely the state the projection exists to prevent — and
// would leave the caches stale. What this body owns is the REFUSALS, which the
// store seam does not make.
