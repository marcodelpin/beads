...
// removed, and the single definition of what `bd ready`'s published total
// means.
//
...
// so both are ErrValidation here rather than accepted and dropped, which is
// the same call ValidateClaimNextRequest makes for the same reason.
//
...
// and the caller's request is not the place to say it.
