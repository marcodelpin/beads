...
func ValidateSettingWrite(key, value string) (string, error) {
	...
	// labels.exclusive-prefixes (bd-7u5ki) gates the label-write choke point
	// (issueops.EnforceExclusiveLabelInTx) on both the classic and proxied
	// stacks, so a malformed value must be refused HERE - the one place both
	// routes call - rather than only on the direct-route front door that used
	// to be the sole caller of labelns.ValidatePrefixes.
