...
func ValidateSettingWrite(key, value string) (string, error) {
	...
	// labels.exclusive-prefixes (bd-7u5ki) is checked here for the same reason
	// status.custom is: this is the ONE validator both issueops.WorkspaceConfig
	// bodies call (internal/storage/uow and internal/workapi/storeworkspaceconfig),
	// and it is what the proxied server's PUT /v0/beads/config/{key} handler
	// runs too - so a malformed prefix list is rejected identically whether it
	// arrives via 'bd config set' on a direct database or through the server.
