...
- **Cross-type blocking dependencies are now allowed** (bd-wg7ve).
  ...
  The guard now also covers `conditional-blocks`, which previously skipped
  cross-type validation entirely. A task gated on an epic becomes ready when
  the epic itself is closed, consistent with `bd ready`/`bd blocked`.
